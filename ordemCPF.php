...
$sql = "SELECT id, cpf FROM Candidato";
...
for ($i = 0; $i < count($vetor); $i++) {
    ...
    $sql = "SELECT id,cpf,nome FROM Candidato WHERE id = '$cadeia'";
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    echo "id: " . $row["id"] . "----" . " CPF: " . $row["cpf"] . "  - Nome: " . $row["nome"] . "<br>";
}
