...
$sql = "SELECT id,clc,cargo FROM Candidato WHERE falta <=> NULL ;";
...
if (mysqli_num_rows($result) > 0) { // quantas linhas tem a variável $result?
    ...
    while ($row = mysqli_fetch_assoc($result)) { // retorna uma linha a cada chamada
        ...
        $clasCargo = strval($row["clc"]);
        $clasCargo = str_pad($clasCargo, 4, '0', STR_PAD_LEFT);

        $vetor[$cont]->campoChave = $cargo . $clasCargo;
        $cont = $cont + 1;
    }
} else {
...
for ($i = 0; $i < count($vetor); $i++) {
    ...
    $sql = "SELECT nome,numInscricao,ano,mes,dia,cargo,ccl FROM Candidato WHERE id = '$cadeia'";
    $result = mysqli_query($conn, $sql);
    ...
    $anterior = $row["cargo"];
    echo $row["cargo"] . "--" . "Num:" . $row["numInscricao"] . "--Nome: " . $row["nome"] . "  --Nascimento:" . $row["dia"] . "/" . $row["mes"] . "/" . $row["ano"];
    if ($row["ccl"] <> 0) {
        echo "---***APROVADO***" . "<br>";
    } else {
        echo "<br>";
